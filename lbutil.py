...
def getlbvs(mongoclient, season, mode, lb, vsplid):
    if mode == "wins":
        matches = list(mongoclient['b2']['matches'].find({'season': season, 'loser': vsplid}))
    if mode == "losses":
        matches = list(mongoclient['b2']['matches'].find({'season': season, 'winner': vsplid}))
    playermap = []
    plidtoint = {}
    pticc = 0
    for i in matches:
    ...
    for i in lb["lb"]:
        if not i['profile'] in plidtoint:
            continue
        plrentry = playermap[plidtoint[i['profile']]]
        if (plrentry['w']+plrentry['l']+plrentry['d']) < 1:
            continue
        tmp = mongoclient["b2"]["players"].find({"plid": i["profile"], "season": season}).sort("date", -1).limit(1)[0]
        tmp["__lbprop"] = getlbprop(mode, plrentry)
        tmp["__lbsort"] = tmp["__lbprop"]
